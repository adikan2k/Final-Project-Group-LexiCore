import os
# --- CRITICAL MAC FIX: MUST BE THE FIRST LINES ---
...
# ============================================================
# PATH CONFIGURATION (ROBUST FILE FINDER)
# ============================================================
WORKING_DIR = Path(".")
...
def find_file(filename):
    ...
    1. Current folder
    2. The 'utils/output_1' folder (based on your screenshot structure)
    3. Common Kaggle paths
    """
    search_paths = [
        Path("."),
        Path("..") / "utils" / "output_1",  # Matches your local directory structure
        Path("utils") / "output_1",
        Path("output_1"),
        Path("/kaggle/input/utils-files/utils/output_1")
    ]
...
# --- REPLACED FAISS WITH NUMPY (STABLE) ---
# This class behaves exactly like FAISS but uses standard math to prevent crashes
class NumpyRetriever:
    ...
    def search(self, query, top_k=10):
        ...
        scores = np.dot(qv, self.embeddings.T)[0]
...
class HybridRetriever:
    def __init__(self, bm25, vector_ret):
        self.bm25, self.vector = bm25, vector_ret
    def search(self, query, top_k=10):
        b_res = dict(self.bm25.search(query, 50))
        f_res = dict(self.vector.search(query, 50))
...
@st.cache_resource
def load_all_data():
    # 1. Load DataFrame (prefer categorized)
    df_path = find_file("papers_with_categories.parquet")
    if df_path is None:
        df_path = find_file("cleaned_papers.parquet")
    
    if df_path is None:
        st.error("❌ CRITICAL ERROR: Could not find data file.")
        st.stop()
        
    df = pd.read_parquet(df_path)
    
    # 2. Load Embeddings
    emb_path = find_file("sbert_abstract_embeddings.npy")
    if emb_path is None:
        st.error("❌ CRITICAL ERROR: Could not find 'sbert_abstract_embeddings.npy'.")
        st.stop()
    ...
    vector_ret = NumpyRetriever(emb) # Using Numpy instead of FAISS
    hybrid = HybridRetriever(bm25, vector_ret)
